...
MarketPulse에서 GCP Pub/Sub을 사용하기 위한 웹 콘솔 설정 방법입니다.
...
4. 프로젝트 이름 입력 (예: `market-pulse`)
5. "만들기" 클릭
6. 생성된 프로젝트 ID 기록 (예: `market-pulse-12345`)
...
4. 서비스 계정 설명 (선택): "MarketPulse Pub/Sub access"
5. "만들기 및 계속하기" 클릭
6. 역할 추가:
   - "역할 선택" 드롭다운 클릭
   - "Pub/Sub 게시자" 선택
...
6. 자동으로 다운로드되는 JSON 파일 저장
   - 예: `market-pulse-12345-abcdef123456.json`
...
GCP_PROJECT_ID=market-pulse-12345
GCP_PUBSUB_TOPIC_ID=prism-trading-signals
GCP_PUBSUB_SUBSCRIPTION_ID=prism-trading-signals-sub
GCP_CREDENTIALS_PATH=/path/to/market-pulse-12345-abcdef123456.json
```
