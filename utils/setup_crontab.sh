...
# =============================================================================
# MarketPulse Crontab 설정 스크립트
# =============================================================================
# 이 스크립트는 MarketPulse 자동 실행을 위한 crontab을 설정합니다.
...
install_crontab() {
    ...
    if crontab -l &> /dev/null; then
        ...
        # 기존 MarketPulse 엔트리 제거
        sed -i '/MarketPulse/,/^$/d' "$temp_cron"
        echo "" >> "$temp_cron"
    else
        > "$temp_cron"
    fi
...
verify_installation() {
    ...
    echo "----------------------------------------"
    crontab -l | grep -A 20 "MarketPulse" || log_warn "MarketPulse 엔트리를 찾을 수 없습니다."
    echo "----------------------------------------"
}
...
interactive_setup() {
    echo -e "${BLUE}================================================${NC}"
    echo -e "${BLUE}   MarketPulse Crontab 설정 도구${NC}"
    echo -e "${BLUE}================================================${NC}"
    echo
...
# 제거 함수
uninstall_crontab() {
    log_info "MarketPulse crontab 엔트리를 제거합니다..."
    ...
    if crontab -l &> /dev/null; then
        ...
        rm -f "$temp_cron"
        log_success "MarketPulse crontab 엔트리가 제거되었습니다."
    else
...
옵션:
  ...
  -u, --uninstall   MarketPulse crontab 엔트리를 제거합니다
...
예시:
  ...
  # 환경 변수를 사용한 자동 설치
  PROJECT_DIR=/opt/market-pulse PYTHON_PATH=/usr/bin/python3 $0 --non-interactive
...
main() {
    ...
    case $action in
        ...
        uninstall)
            if $interactive; then
                read -p "정말로 MarketPulse crontab 엔트리를 제거하시겠습니까? (y/N): " confirm
                if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
                    log_info "취소되었습니다."
                    exit 0
                fi
        ...
        show)
            log_info "현재 설치된 MarketPulse crontab:"
            echo "----------------------------------------"
            crontab -l 2>/dev/null | grep -A 30 "MarketPulse" || log_warn "MarketPulse 엔트리가 없습니다."
            ;;
