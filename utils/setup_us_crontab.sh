...
# =============================================================================
# MarketPulse US Stock Market Crontab Setup Script
# =============================================================================
...
generate_crontab_entries() {
    cat << EOF
# =============================================================================
# MarketPulse US Stock Market Auto-Execution Schedule
...
install_crontab() {
    ...
    if crontab -l &> /dev/null; then
        ...
        sed -i '/MarketPulse US Stock/,/^$/d' "$temp_cron"
        echo "" >> "$temp_cron"
    else
        > "$temp_cron"
    fi
...
verify_installation() {
    ...
    echo "----------------------------------------"
    crontab -l | grep -A 30 "MarketPulse US" || log_warn "No US entries found"
    echo "----------------------------------------"
}

uninstall_crontab() {
    log_info "Removing MarketPulse US crontab entries..."
...
Examples:
  ...
  PROJECT_DIR=/opt/market-pulse TIMEZONE_MODE=EDT $0 --non-interactive
...
interactive_setup() {
    echo -e "${BLUE}================================================${NC}"
    echo -e "${BLUE}   MarketPulse US Crontab Setup Tool${NC}"
    echo -e "${BLUE}================================================${NC}"
    echo
...
main() {
    ...
    case $action in
        ...
        show)
            ...
            echo "----------------------------------------"
            crontab -l 2>/dev/null | grep -A 30 "MarketPulse US" || log_warn "No US entries found"
            ;;
