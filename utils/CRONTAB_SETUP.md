...
PROJECT_DIR=/opt/market-pulse PYTHON_PATH=/usr/bin/python3 ./setup_crontab.sh --non-interactive
```
...
PYTHONPATH=/path/to/market-pulse
```
...
0 2 * * * chmod +x /path/to/market-pulse/utils/backup_configs.sh && /path/to/market-pulse/utils/backup_configs.sh
...
0 3 * * * cd /path/to/market-pulse && utils/cleanup_logs.sh
...
0 7 * * 1-5 cd /path/to/market-pulse && python update_stock_data.py >> logs/update.log 2>&1

# Morning analysis (Mon-Fri 9:30 AM)
30 9 * * 1-5 cd /path/to/market-pulse && python stock_analysis_orchestrator.py --mode morning >> logs/morning.log 2>&1

# Afternoon analysis (Mon-Fri 3:40 PM)
40 15 * * 1-5 cd /path/to/market-pulse && python stock_analysis_orchestrator.py --mode afternoon >> logs/afternoon.log 2>&1
...
5 11 * * 1-5 cd /path/to/market-pulse && python examples/generate_dashboard_json.py >> logs/generate_dashboard_json.log 2>&1

# Performance tracker daily update (Mon-Fri 5 PM) - must run before evening dashboard refresh
0 17 * * 1-5 cd /path/to/market-pulse && python performance_tracker_batch.py >> logs/performance_tracker.log 2>&1

# Dashboard JSON refresh - Afternoon (Mon-Fri 5:10 PM)
10 17 * * 1-5 cd /path/to/market-pulse && python examples/generate_dashboard_json.py >> logs/generate_dashboard_json.log 2>&1
```
...
sudo semanage fcontext -a -t bin_t "/path/to/market-pulse/.*\.py"
sudo restorecon -Rv /path/to/market-pulse/
...
# Create ~/Library/LaunchAgents/com.market-pulse.plist
```
...
chown -R $USER:$USER /path/to/market-pulse
```
...
```bash
# Create /etc/logrotate.d/market-pulse
/path/to/market-pulse/logs/*.log {
    daily
    missingok
    rotate 30
    compress
...
1. Inquire on [GitHub Issues](https://github.com/yourusername/market-pulse/issues)
